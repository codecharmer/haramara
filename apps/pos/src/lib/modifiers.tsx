...
import { color, money, radius, space, type } from './theme';
...
/** One resolved modifier group (Catalog\ModifierResolver serialization). */
export interface ModifierGroup {
	id: number;
	name: string;
	/** Minimum selections once the group is engaged. */
	min: number;
	/** Maximum selections; 0 = no limit. 1 renders as single-select. */
	max: number;
	/** Required groups cannot be skipped. */
	required: boolean;
	options: ModifierOption[];
}

/** What the POS sends per line: chosen option keys per group. */
